...
format_vdem <- function(dat, keep = c("COWcode", "country_name", "year", used_vars)){
  ...
  dat <- dat[year >= 1934 & year <= 2023]   # Filter rows for years between 1934 and 2023
  ...
  names(dat)[names(dat) == 'v2csreprss_ord'] <- 'repress_civilsoc'
  ...
  dat$repress_civilsoc = as.numeric(dat$repress_civilsoc==0)
