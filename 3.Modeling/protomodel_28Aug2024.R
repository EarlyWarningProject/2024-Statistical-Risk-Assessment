...
# Read in data ====
alldata = read.csv("prepared2023predictors-2024-08-28.csv")
...
predictornames.fullest <- c("widetargeting", "narrowtargeting", "womencivsoc", 
                            ...
                            "un_imr_sqrt", "efindex", "discrimpop", "reg.na", "reg.eap", 
...
predictornames.offsetdecay <- c("widetargeting", "narrowtargeting", "womencivsoc", 
                                "govcensorship", "partyban.new.0", "partyban.new.1", 
                                "partyban.new.2", "minorityrule", "judicialreform", 
                                "religiousfreedom", "pol_killing_approved", "freediscussion", 
                                "social_inequality", "even_civilrights", "repress_civilsoc", 
...
saveRDS(model.full.out$elastic.cv, file = "main_full_cv_glmnet_object_31Aug2024.rds")
...
library(data.table)
fwrite(drop_out_df_sorted, "reg_model_dropouts.csv")
...
# Save the dataframe to a CSV file
fwrite(od_drop_out_df_sorted, "od_model_dropouts.csv")
...
fwrite(merged_drop_out_df, "merged_model_dropouts.csv")
...
write.csv(allforecasts, file=paste0("simple_FOOSforecasts_",savenameaffix,".csv"))
...
distplot = ggplot(allforecasts, aes(x = maxrisk2, fill = as.factor(mk_onset))) +
  ...
  labs(fill = "Group",
       x = "Risk",
       y = "Density",
       title = "Density of Risk by Group - Simple Model",
       subtitle = "Grouped by onset following") +
  theme_minimal()

print(distplot)
ggsave(paste0("simple_distplotFOOS_maxrisk2_",savenameaffix,".pdf"), plot = distplot, width=7, height=5, dpi=300)
...
catchplot = ggplot(onsetforecasts, aes(x = maxrisk2)) +
  ...
  labs(title = "Simple Model - Empirical CDF of Risk for Countries with onsets after",
       x = "risk",
       y = "fraction caught") +
  theme_minimal() +
  scale_x_reverse() +
...
print(catchplot)
ggsave(filename = paste0("simple_catchplot_",savenameaffix,".pdf"), catchplot)
