...
# Read in data ====
alldata = read.csv("prepared2023predictors-2024-09-05.csv")
...
saveRDS(model.full.out$elastic.cv, file = "main_full_cv_glmnet_object_5Sep2024.rds")
...
library(data.table)
fwrite(drop_out_df_sorted, "reg_model_dropouts_5Sep.csv")
...
# Save the dataframe to a CSV file
fwrite(od_drop_out_df_sorted, "od_model_dropouts_5Sep.csv")
...
fwrite(merged_drop_out_df, "merged_model_dropouts_5Sep.csv")
...
write.csv(allforecasts, file=paste0("od_FOOSforecasts_",savenameaffix,".csv"))
...
print(distplot)
ggsave(paste0("od_distplotFOOS_maxrisk2_",savenameaffix,".pdf"), plot = distplot, width=7, height=5, dpi=300)
...
print(catchplot)
ggsave(filename = paste0("od_catchplot_",savenameaffix,".pdf"), catchplot)
